...
# These plots are generated by DVCLive in Vega format, so no need to also log them as images.
PLOTS_TO_SKIP = ("confusion_matrix.png", "confusion_matrix_normalized.png", "results.png")
...
def _log_plots(plots, prefix=''):
    for image_path, params in plots.items():
        if image_path.name in PLOTS_TO_SKIP:
            continue
        timestamp = params['timestamp']
        if _processed_plots.get(image_path) != timestamp:
            _log_images(image_path, prefix)
            _processed_plots[image_path] = timestamp
...
def _log_confusion_matrix(validator):
    ...
    live.log_sklearn_plot('confusion_matrix', targets, preds, normalized=False)
    live.log_sklearn_plot('confusion_matrix', targets, preds, normalized=True)
